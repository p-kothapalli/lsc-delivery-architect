...
> **Read this before installing anything.** The first two rows are **internal
> servers that are not publicly available**. A fresh install has neither, and
> needs neither — the "Substitute" column is the supported path, requires no
> setup, and is what produced every artifact this skill has shipped. If an agent
> tells you to obtain these server definitions from the author, that is wrong:
> there is nothing to obtain.

| Alias used in this skill | Resolved ID | Last observed (2026-09-22) | Substitute when absent (no setup required) | Use for |
|---|---|---|---|---|
| **`code-review-graph`** (`semantic_search_nodes`, `query_graph`, `get_impact_radius`, `get_affected_flows`, `get_review_context`) | **internal — not publicly distributed** | **never registered** in the authoring workspace | **Grep / Glob / Read over the repo.** This is the operative path for STEP 3, not a degraded one — finding nothing this way is not proof of absence. | Verify **custom** components, trace chains, Impact Analysis. |
| **`salesforce-docs`** (`salesforce_docs_search`, `salesforce_docs_fetch`) | `user-salesforce-docs` — **internal, requires SSO** | **error state** in the authoring workspace on 2026-08-06, 08-31, 09-10 and 09-22 (47 days and counting) | The official **`help.salesforce.com`** and **`developer.salesforce.com`** pages, fetched directly. They are authoritative, public, and reachable. Cite the URL. | Verify **standard** Salesforce / Life Sciences Cloud object/field/API/feature facts with citations. |
| **Salesforce DX MCP** — the public option, `@salesforce/mcp` | `npx -y @salesforce/mcp@latest --orgs DEFAULT_TARGET_ORG --toolsets orgs,metadata,data,users,testing` | official, public, npm | *(this **is** the substitute — optional)* | **Strongest evidence available:** `run_soql_query` and `retrieve_metadata` against an org with LSC deployed answer "does this object/field exist *here*" better than any doc can. Optional, but the one worth configuring. |
